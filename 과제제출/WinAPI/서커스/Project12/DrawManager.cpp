...
void DrawManager::DrawWinImages(HDC hdc, const int& CharacterXLocation, const int& CharacterYLocation, const int& Life, const int& Score, const int& BonusScore)
{
	HBITMAP BitMapBack = CreateDIBSectionRe(hdc, m_WindowWidth, m_WindowHeight);
	HBITMAP OldBitMapBack = (HBITMAP)SelectObject(m_MemDCBack, BitMapBack);

	Map->DrawWin(m_MemDCBack);
	CharacterObject->DrawWin(m_MemDCBack, CharacterXLocation, CharacterYLocation);
