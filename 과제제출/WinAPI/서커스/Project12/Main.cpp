...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		SetTimer(hWnd, 1, 30, NULL);	//캐릭터 프레임 조절용
...
이거 다하고 충돌체크 다시 구현하고... 점수랑 라이프 어저고저저고

히트 박스 개선하고, 화로 분리하고, 점수, 목숨, 골 구현하면 끝

점수 증가 구현 거의 다 됨... 허점 하나 때문에 시간 다 날린 거 실화냐??
화로 분리, 판정 완화, 게임 오버, 골 구현하면 끝


골 지점이 보일때 더 이상 스크롤 안 하도록 if문을 처리 해야 함..->배경 오브젝트 쪽에 넣는 게 나을 듯하다
게임오버는 거창하게 안 하고 목숨 다 되면 게임 오버 화면에 띄우고 끄게 만들어버리기

판정 완화 완료... 화로 분리, 게임 오버, 골 구현 남음

원판은 걍 화로를 넘는다는 것 자체에 점수 보너스를 부여했는지 같은 화로를 무한대로 넘어제끼면 무한대로 점수를 올릴 수 있음 물론 이론상
왜 화로배열을 만들 수 없는가........
