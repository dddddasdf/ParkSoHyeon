...
void Play::PlayMain(string &NameTmp, int &ScoreTmp, int &StageTmp)
{
	...
	GetInstance()->GetName();
	PrintBottomArea();
	GetInstance()->PrintLife();
	GetInstance()->PrintScore();
...
void Play::Init()
{
	GetInstance()->m_iLife = 10;
	GetInstance()->m_iScore = 0;
	GetInstance()->m_sUserName = "\? \? \?";
	GetInstance()->m_iStageNumber = 1;
	GetInstance()->m_iNumberOfKilledEnemy = 0;
	GetInstance()->m_iSpawnSpeed = 2500;
	GetInstance()->m_iMovingSpeed = 600;
}
...
void Play::PrintLife()
{
	...
	for (int i = 0; i < GetInstance()->m_iLife; i++)
		cout << "♥";
