...
int Weapon::PrintWeaponList(int HaveGold, int WeaponType, int WeaponNumber, int &ChangeWeaponType, int &ChangeWeaponIndex, WeaponStruct *TypeWeapon)
{
	...
	WeaponStack StackTmp;
	StackTmp.WeaponArray = new WeaponStruct[WeaponNumber];
	StackTmp.iTopIndex = -1;
	...
	while (1)
	{
		...
		if (StackTmp.iTopIndex == -1)
			PushWeaponStack(&StackTmp, TypeWeapon, WeaponType, WeaponNumber, &iYPos);
		else
			PopWeaponStack(&StackTmp, &iYPos);
		...
		iSelect = MenuSelectCursor(WeaponNumber + 4, 3, 3, 1);
...
void Weapon::PushWeaponStack(WeaponStack *Array, WeaponStruct *TypeWeapon, int WeaponType, int WeaponNumber, int *YPos)
{
	for (int i = 0; i < WeaponNumber; i++)
	{
		gotoxy(10, *YPos);
		...
		*(YPos) += 2;
		...
		Array->iTopIndex += 1;
		Array->WeaponArray[Array->iTopIndex] = TypeWeapon[i];
	}
}

void Weapon::PopWeaponStack(WeaponStack *Array, int *YPos)
{
	for (; ; )
	{
		gotoxy(10, *YPos);
		cout << "가격: " << Array->WeaponArray[Array->iTopIndex].iWeaponPrice;
		gotoxy(35, *YPos);
		cout << "무기 타입: ";
		if (Array->WeaponArray[Array->iTopIndex].iWeaponType == TYPE_DAGGER)
			cout << "단검";
		else if (Array->WeaponArray[Array->iTopIndex].iWeaponType == TYPE_GUN)
			cout << "총";
		else if (Array->WeaponArray[Array->iTopIndex].iWeaponType == TYPE_SWORD)
			cout << "칼";
		else if (Array->WeaponArray[Array->iTopIndex].iWeaponType == TYPE_WAND)
			cout << "원드";
		else if (Array->WeaponArray[Array->iTopIndex].iWeaponType == TYPE_BOW)
			cout << "활";
		else if (Array->WeaponArray[Array->iTopIndex].iWeaponType == TYPE_HAMMER)
			cout << "둔기";
		*YPos += 1;
		gotoxy(10, *YPos);
		cout << "무기명: " << Array->WeaponArray[Array->iTopIndex].sWeaponName;
		gotoxy(35, *YPos);
		cout << "공격력: " << Array->WeaponArray[Array->iTopIndex].iWeaponPower;

		*(YPos) += 2;

		Array->iTopIndex -= 1;

		if (Array->iTopIndex == -1)
			break;
	}
}
