...
void Play::PrintSynopsis()
{
	...
	if (SynopsisText.is_open())
	{
		...
		int iLines;	//총 줄 개수
		string sTmp;	//문자열 임시로 담아줄 놈

		SynopsisQueue *NewQueue = new SynopsisQueue;
		SynopsisNode *NodeTmp;
		NewQueue->Front = NULL;
		NewQueue->Rear = NULL;
		...
		while (1)
		{
			...
			if (!SynopsisText.eof())
			{
				getline(SynopsisText, sTmp);
				EnQueueSynopsis(NewQueue, sTmp);
			}
			else
			{
			...
			{
				int YTmp = 0;
				NodeTmp = NewQueue->Front;
				for (; NodeTmp != NULL; YTmp++)
				{
					gotoxy(WIDTH - (NodeTmp->sSentence.length() / 2), iTopY + YTmp);
					cout << NodeTmp->sSentence;
...
void Play::EnQueueSynopsis(SynopsisQueue *Queue, string NewSentence)
{
	SynopsisNode *NewNode = new SynopsisNode;
	NewNode->sSentence = NewSentence;
	NewNode->Next = NULL;
...
void Play::DeQueueSynopsis(SynopsisQueue *Queue)
{
	if (Queue->Front == NULL)
		;
	else
	{
		SynopsisNode *DiscardNode = Queue->Front;
